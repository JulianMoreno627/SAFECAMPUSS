...
const _tipos = ['Todos', 'Robo', 'Acoso', 'Pelea', 'Vandalismo', 'Accidente', 'Persona sospechosa', 'Iluminación', 'Otro'];

class ListaReportesScreen extends ConsumerStatefulWidget {
  const ListaReportesScreen({super.key});

  @override
  ConsumerState<ListaReportesScreen> createState() => _ListaReportesScreenState();
}
...
class _ListaReportesScreenState extends ConsumerState<ListaReportesScreen> {
  ...
  Future<void> _refresh() async {
    ...
    if (loc != null) {
      await ref.read(reportsProvider.notifier).fetchNearbyReports(loc.latitude, loc.longitude);
    }
  }
  ...
  Widget _buildList(List<dynamic> items) {
    return ListView.builder(
      ...
      itemBuilder: (context, i) => _ReportCard(
        report: items[i],
        onTap: () => ReporteDetalleSheet.show(context, Map<String, dynamic>.from(items[i])),
      ),
    );
  }
  ...
  Widget _buildEmpty() {
    final noResults = _searchQuery.isNotEmpty || _selectedTipo != 'Todos';
    return ListView(
      physics: const AlwaysScrollableScrollPhysics(),
      children: [
        ...
        Center(
          child: Column(
            children: [
              Icon(
                noResults ? Icons.search_off_rounded : Icons.check_circle_outline_rounded,
                color: noResults ? Colors.white24 : AppColors.riskLow,
                size: 64,
              ),
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      if (report['testigos'] != null && report['testigos'] != 0) ...[
                        const SizedBox(width: 8),
                        const Icon(Icons.group_rounded, size: 12, color: Colors.white38),
